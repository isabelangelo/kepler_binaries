...
binary_flux_df = load_spectrum_df('./data/spectrum_dataframes/kraus_binary_flux_dwt.csv')
binary_sigma_df = load_spectrum_df('./data/spectrum_dataframes/kraus_binary_sigma_dwt.csv')
...
def metric_df(flux_df, sigma_df):
	...
	for star in flux_df.columns[1:].to_numpy():
	    ...
	    print('added metrics for {}'.format(star))
...
single_metric_df.to_csv('./data/label_and_metric_dataframes/raghavan_single_metrics.csv')
...
binary_metric_df.to_csv('./data/label_and_metric_dataframes/kraus_binary_metrics.csv')
